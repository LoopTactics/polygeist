...
//#include "Standalone/tc/lang/prefixed_ostream.h"
#include "Standalone/tc/lang/builtins.h"
#include "Standalone/tc/lang/error_report.h"
//#include "Standalone/tc/lang/inference/ranges.h"
#include "Standalone/tc/lang/tree.h"
#include "Standalone/tc/lang/tree_views.h"
#include "Standalone/tc/utils/compiler_options.h"
  ...
  TreeRef checkExp(TreeRef exp, bool allow_access) {
    ...
    case TK_APPLY: {
      ...
      /*
            {
              auto tt = type;
              auto output_annotation =
         annotated_output_types.find(ident.name()); if (output_annotation !=
         annotated_output_types.end()) tt =
              ...
              for (int i = 0; i < type.dims().size(); i++) {
                ranges_to_infer.addConstraints(
                    ...
                    teckyl::ranges::Expr::fromTreeRef(a.arguments()[i],
                                                      rangeParameters),
                    teckyl::ranges::Expr::fromTreeRef(tt.dims()[i],
         rangeParameters));
              }
            }
      */
